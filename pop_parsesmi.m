% pop_parsesmi() - parses text-converted (ASCII) SMI raw data and 
...
try
    ...
    if ~useMessageTrigger
        et = parsesmi( fileToLoad, matToSave);
    else
        et = parsesmi( fileToLoad, matToSave, triggerKeyword);
    end
...
com = sprintf('ET = %s(%s);','parsesmi',allArgs);
return;
