%  eegplugin_eye_eeg() - Preprocess, synchronize, artifact-correct, 
%            and analyze simultaneously recorded eye tracking and EEG data
%
%  Description:
...
%   <a href="matlab:helpwin pop_detecteyemovements">detecteogspikes</a>   - estimate saccade onsets based on spikes in the radial EOG
...
%   <a href="matlab:helpwin detecteyemovements">detecteogspikes</a>     - see pop function
...
%   <a href="matlab:helpwin mergesacc">filtSRP</a>              - filter%   rEOG (by convolution with SP template) (**)
%
...
%
%  (**) function written by Alon Keren, see Keren et al., 2010, NeuroImage
%
%   Third-party helper functions
%   ----------------------------------------------------
%   <a href="matlab:helpwin parseq">parseq</a>                 - Copyright: R. Kliegl & J. Laubrock, University of Potsdam
%   <a href="matlab:helpwin nearestpoint">searchclosest</a>    - Copyright: Jos van der Geest, see included license file 
%
%   Functions that control GUI dialogues
%   ----------------------------------------------------
...
%   <a href="matlab:helpwin dlg_detecteogspikes">dlg_detecteogspikes</a>
%   ----------------------------------------------------
%
...
% Tobii (TM)   is a trademark of Tobii AB, Stockholm, Sweden
...
% ADVANCED
cb_checksync        = [try_strings.check_data, '[EEG LASTCOM] = pop_checksync(EEG);', catch_strings.add_to_hist];
cb_radialeog        = [try_strings.check_data, '[EEG LASTCOM] = pop_detecteogspikes(EEG);', catch_strings.add_to_hist];

pause(0.001)
% Note: This pause is due to one of the weirdest bug I have ever tracked.
% If ERPLAB is also in the plugins folder, the menu order gets messed up
% of the main EEGLAB menus (File, ..., ..., Help etc.) causing EYE-EEG's 
...
%uimenu( parseEyetracker_m, 'Label', 'EDF file from Eyelink', 'CallBack', cb_parseeyelink_edf);
...
advanced_m = uimenu( eyetracker_m, 'Label', 'Special-purpose functions', 'Separator', 'on',  'userdata', 'startup:off;');
uimenu(advanced_m, 'Label', 'Evaluate ET/EEG synchronization (cross-corr.)', 'CallBack', cb_checksync, 'Separator', 'off', 'userdata', 'startup:off;'); % epoch:off;'  
uimenu(advanced_m, 'Label', 'Detect saccade onsets based on (radial) EOG spikes', 'CallBack', cb_radialeog, 'Separator', 'off', 'userdata', 'startup:off;'); % epoch:off;'  
